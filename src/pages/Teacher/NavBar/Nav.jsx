...
const Nav = () => {
  // Sidebar state manage karne ke liye
  ...
  return (
    <>
      {/* 1. Open/Hamburger Icon (Sirf icon, no text) */}
      {!isOpen && (
        <button 
      ...
      {/* 2. Sidebar Component */}
      <div className={`fixed top-0 left-0 h-full lg:w-[230px] w-[200px] bg-white border-r border-slate-100 py-8 px-1 z-[100] flex flex-col justify-between shadow-sm overflow-y-auto transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        
        {/* Close Button (Sirf Mobile ke liye) */}
        <button 
          onClick={() => setIsOpen(false)}
          className="absolute top-4 right-4 md:hidden p-2 text-slate-500 hover:text-black"
        >
