...
function Dashboard() {
  // Pure Local Static Fallbacks for Application Identity
  const [name] = useState("Ikram Khan");
  const [currentDate, setCurrentDate] = useState('');

  // Local Static Mock Dataset Matrix for Global Institute Boards
  const [institutenotices] = useState([
    {
      I_date: "2026-05-20",
      I_topic: "Annual Hackathon Registration Open",
      I_description: "Teams across Full-Stack tracks can lock registrations inside the MERN portal starting today. Zero late registration concessions will be permitted."
    },
    {
      I_date: "2026-05-18",
      I_topic: "Server Room Infrastructure Maintenance Window",
      I_description: "Local dev proxy links and authentication endpoints under the 5000 pipeline will undergo standard service reboots."
    },
    {
      I_date: "2026-05-15",
      I_topic: "E-Wallet Verification Cycle Deadlines",
      I_description: "All academic students must dispatch structural clearance criteria files via the direct portal to reactivate internal payment tokens."
    }
  ]);

  // Real-time Static Calendar Initializations
  useEffect(() => {
  ...
  return (
    <div className="w-full bg-slate-50 min-h-screen pb-12">
      ...
      {/* Main Structural Layout Canvas with Left Sidebar Padding Buffer */}
      <div className="w-full max-w-[1300px] mx-auto px-4 pl-4 md:pl-[290px] mt-8 transition-all space-y-6">
        
        {/* Top Hero Accent Box Banner Segment */}
        <div className="relative w-full bg-gradient-to-r from-[#1E2D42] via-[#2F4563] to-[#384D6C] border-2 border-slate-900 rounded-[22px] p-6 md:p-8 text-white overflow-hidden shadow-sm">
          ...
          {/* Main Context Typography Fields */}
          <div className="relative z-10 space-y-2 max-w-xl">
            ...
            <p className="text-xs md:text-sm text-slate-300 font-medium max-w-md leading-relaxed">
              System access approved. Your local MERN static interface engine is active. Monitor your course updates and notices below.
            </p>
          </div>

          {/* Decorative Vector Branding Absolute Background Nodes */}
          <div className="absolute right-0 top-0 h-full w-1/3 opacity-25 md:opacity-100 hidden sm:block pointer-events-none">
            <img 
              src={dashboardImg} 
              alt="Dashboard Graphic Art" 
              className="w-full h-full object-cover object-center transform scale-105 transition-transform"
            />
          </div>
          
          {/* Micro Ambient Light Effect */}
        ...
        <div className="bg-white border-2 border-slate-900 rounded-[22px] p-5 md:p-6 shadow-sm space-y-6">
          ...
          {/* Dynamic Loop Mapping Over Mock Arrays */}
          {institutenotices.length > 0 ? (
            ...
            <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                No active institutional announcements linked inside this component branch.
              </p>
            </div>
          )}
