...
const SignUp = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Unified Architecture Backend Port 3000
    const url = `http://localhost:3000/api/auth/register`;

    fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include", // 🌟 CRITICAL: Cookies aur Headers cross-origin allow karne ke liye
  ...
  const signUpWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      // 🔥 FIXED: Port 5000 se badal kar standard 3000 kiya taaki unified server hit ho
      fetch(`http://localhost:3000/api/auth/google-auth`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // 🌟 CRITICAL: Google flow mein bhi credentials lazmi hain
        body: JSON.stringify({ 
